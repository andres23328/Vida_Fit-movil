...
router.get('/predict', async (req, res) => {
    try {
        ...
        usuarios = todosUsuarios.map(user => ({
            ...user,
            gender: user.gender === 'Hombre' ? 1 : 2,
            location: user.location === 'Casa' ? 1 : 2,
            units: user.units === 'metric' ? 1 : 2,
            goal: user.goal === "Ganar Músculo" ? 3 
                : user.goal === "Perder Grasa y Ganar Músculo" ? 1
                : user.goal === "Perder Grasa" ? 2 
                : 0 
        }));
        ...
        const featuresKmeans = usuarios.map(user => [
            ...
            parseInt(user.age),               
            user.location,   // 🔹 YA ES NÚMERO, NO NECESITA parseInt()
            user.goal,       // 🔹 TAMBIÉN YA SE CONVIRTIÓ EN NÚMERO ANTES
            parseInt(user.exerciseLevel),     
            user.gender,     // 🔹 YA ES NÚMERO, NO NECESITA parseInt()
            parseInt(user.trainingIntensity),
            parseInt(user.strength),          
            parseInt(user.combinedTraining),  
            parseInt(user.totalHealth),       
            parseInt(user.cognitiveHealth),   
            parseInt(user.sexualHealth)       
        ]);
        ...
        try {
            ...
            usuarios.forEach((user, idx) => user.grupo_kmeans = clusters[idx]);
             ...
             const nuevoUsuarioTransformado = {
                ...
                gender: nuevoUsuario[0].gender === 'Hombre' ? 1 : 2,
                location: nuevoUsuario[0].location === 'Casa' ? 1 : 2,
                units: nuevoUsuario[0].units === 'metric' ? 1 : 2,
                goal: nuevoUsuario[0].goal === "Ganar Músculo" ? 3 
                    : nuevoUsuario[0].goal === "Perder Grasa y Ganar Músculo" ? 1
                    : nuevoUsuario[0].goal === "Perder Grasa" ? 2 
                    : 0
            };
            
            // Características del nuevo usuario para la predicción
            const nuevoUsuarioFeatures = [
                parseFloat(nuevoUsuarioTransformado.height),  
                nuevoUsuarioTransformado.units,        
                parseFloat(nuevoUsuarioTransformado.weight),          
                parseInt(nuevoUsuarioTransformado.age),               
                nuevoUsuarioTransformado.location,   // 🔹 YA ES NÚMERO, NO NECESITA parseInt()
                nuevoUsuarioTransformado.goal,       // 🔹 YA ES NÚMERO, NO NECESITA parseInt()
                parseInt(nuevoUsuarioTransformado.exerciseLevel),     
                nuevoUsuarioTransformado.gender,     // 🔹 YA ES NÚMERO, NO NECESITA parseInt()
                parseInt(nuevoUsuarioTransformado.trainingIntensity),
                parseInt(nuevoUsuarioTransformado.strength),          
                parseInt(nuevoUsuarioTransformado.combinedTraining),  
                parseInt(nuevoUsuarioTransformado.totalHealth),       
                parseInt(nuevoUsuarioTransformado.cognitiveHealth),   
                parseInt(nuevoUsuarioTransformado.sexualHealth)      
            ];
